...
void Pattern0::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
...
void Pattern1::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t r = context.pixelCoordsPolar[i][0];
...
void Pattern2::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
...
void Pattern3::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t x = context.pixelCoords[i][0];
...
void Pattern4::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t r = context.pixelCoordsPolar[i][0];
...
void Pattern5::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t r = context.pixelCoordsPolar[i][0];
...
void Pattern6::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t r = context.pixelCoordsPolar[i][0];
...
void Pattern7::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t x = context.pixelCoords[i][0];
...
void Pattern8::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t r = context.pixelCoordsPolar[i][0];
...
void Pattern9::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  for (uint8_t i = 0; i < NUM_PIXELS; ++i)
  {
    uint8_t x = context.pixelCoords[i][0];
...
void Pattern10::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  ...
  for (int i = 0; i < 20; ++i)
  {
    CRGB color = ColorFromPalette(*palette, sin8(context.now / 15 + i * 5));

    float x = p->width() / 2.0;
    float y = p->height() / 2.0;
    float r = p->width() / (1.0 + i / 2.0);

    float angle = (float)context.now / 3000.0f * M_PI * 2.0 + (float)i / 20.0 * 1.5;
...
void Pattern11::draw(CRGB *leds, LEDContext &context, CRGBPalette16 *palette)
{
  LEDPanel *p = context.panel;
  p->fillScreen(0);
